...
import grpc
from generated_pb2 import ledger_pb2_grpc, ledger_pb2


class HealthService(ledger_pb2_grpc.HealthServicer):
    """
    ...
    def Check(self, request, context):
        ...
        return ledger_pb2.HealthCheckResponse(status=ledger_pb2.HealthCheckResponse.SERVING)
    ...
    def Watch(self, request, context):
        ...
        while True:
            yield ledger_pb2.HealthCheckResponse(status=ledger_pb2.HealthCheckResponse.SERVING)


def serve():
    """
    Start the health check server on port 50053.
    
    This server runs alongside the main service and exposes health checks.
    """
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=2))
    ledger_pb2_grpc.add_HealthServicer_to_server(
        HealthService(), server
    )
    server.add_insecure_port("[::]:50053")
    server.start()
    print("gRPC Health Check Service running on port 50053")
    return server
